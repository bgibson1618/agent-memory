...
| `okf` | The OKF schema contract: frontmatter (id/slug, title, description, type, `topics[]`, `sensitivity`, timestamps, `related[]`, `source` — provenance citation, D9; a mem key, not OKF v0.1 vocabulary, like `related`) + body with `[[wikilinks]]` | Conformance verified against the **official OKF v0.1 spec** (snapshot: `research/okf-spec-v0.1.md`; D2/D5). Interop: serializer mirrors the spec-recommended `tags`/`timestamp`, parser accepts them as fallback for external files, and the reserved filenames `index`/`log` are refused as concept slugs |
...
| `fusion` | RRF across the three legs; rerank seam (no-op in v1); non-`normal` sensitivity marking (`[work]`/`[sensitive]`, D16) + `--no-work`/`--no-sensitive` filters | Write-to-all + fuse-on-read is settled design (capstone D021/D023/D045) |
...
→ RRF fuse (k=60, unweighted v1) → rerank seam (no-op) → mark [work]/[sensitive] → text/JSON out
```
...
- **Egress guard mechanism**: the zero-egress proof (F12) runs the full command surface —
  ...
  preflight); "full" is not aspirational: a parity test asserts the driver exercises every
  CLI subcommand, so the proof grows with the surface (2026-08-05 checkpoint remediation).
